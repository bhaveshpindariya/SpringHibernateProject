...
<html lang="en">
	...
	<div id="wrapper">
      ...
      <div id="page-wrapper"> 
      		 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Signature Role Details</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example" style="font-size:13px;">
							        ...
							        <tbody>
							            <c:forEach items="${master}" var="signRol" varStatus="status">     
							  			<tr>
							            	<td><c:out value="${signRol.no}"/></td>
							                <td><c:out value="${signRol.name}"/></td>
							                <td><c:out value="${signRol.createdBy}"/></td>
							                <td><c:out value="${signRol.createdDate}"/></td>
							                <td><c:out value="${signRol.modifyBy}"/></td>
							                <td><c:out value="${signRol.modifyDate}"/></td>
							                <td><a href="#" onclick="editData('${signRol.id}','${signRol.name}')" data-toggle="modal" rel="tooltip" data-target="#exampleModal"  title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="deleteData('${signRol.id}')" title="Delete" data-toggle="modal" rel="tooltip" data-target="#deletbox"><span class="glyphicon glyphicon-trash"></span></a></td>
							            </tr>   
									</c:forEach>
							        </tbody>
                                </table>
  ...
  <div class="modal-dialog" >
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">Edit signature role</h4>
      </div>
      <div class="modal-body">
        <form>
          ...
          <div class="form-group">
            <label for="message-text" class="control-label">Signature role name</label>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">Add signature role</h4>
      </div>
      <div class="modal-body">
        <form>
        <div class="form-group">
        ...
        <div class="dataTable_wrapper">
           <table class="table table-striped table-bordered table-hover" id="dataTables-example1" style="font-size:13px;">
               <thead>
      		   ...
      		   <tbody>
          			<c:forEach items="${masters}" var="srom" varStatus="status">     
 						<tr>
           					<td><c:out value="${srom.no}"/></td>
			                <td><c:out value="${srom.name}"/></td>
			                <td><c:out value="${srom.createdBy}"/></td>
			                <td><c:out value="${srom.createdDate}"/></td>
			                <td><c:out value="${srom.modifyBy}"/></td>
			                <td><c:out value="${srom.modifyDate}"/></td>
               				<td><a href="#" onclick="active('${srom.id}')" title="Active" data-toggle="modal" rel="tooltip"><span style="color:green;font-size: 20px;" class="glyphicon glyphicon-ok-circle"></span></a></td>
          				</tr>   
    				</c:forEach>
      			</tbody>
           </table>
...
function reload(){
	...
	$.ajax(
	  {
        ...
        success: function (item) 
        {
        	...
        	{
           		s=true;
           		table.fnAddData([item[i].no,item[i].name,item[i].createdBy,item[i].createdDate,item[i].modifyBy,item[i].modifyDate,'<a href="#" onclick="editData('+item[i].id+',\'' +item[i].name+ '\')" data-toggle="modal" rel="tooltip" data-target="#exampleModal"  title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="deleteData('+item[i].id+')"  title="Inactive" data-toggle="modal" rel="tooltip" data-target="#deletbox"><span class="glyphicon glyphicon-trash"></span></a>']);           
        	}
        },
	...
	$.ajax({
        ...
        success: function (item) {
        	for(var i=0;i < item.length; i++){
           		ss=true;
   	    		table1.fnAddData([item[i].no,item[i].name,item[i].createdBy,item[i].createdDate,item[i].modifyBy,item[i].modifyDate,'<a href="#" onclick="active('+item[i].id+')" title="Active" data-toggle="modal" rel="tooltip"><span style="color:green;font-size: 20px;" class="glyphicon glyphicon-ok-circle"></span></a>']);           
        	}
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            alert(errorThrown);
...
function valides(){
	if($("#printName").val()=="" || $("#printName").val()=="null"){
		...
		$("#dataes1").html("Please enter signature role name");
    	$('#alerts').modal('show');
		return false;
	}else{
...
function deleteData(id){
	$("#id1").val(id);
	$("#exampleModalLabelTitle").html("Inactive signature role detail");
