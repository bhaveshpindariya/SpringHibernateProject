...
<html lang="en">
	...
	<div id="wrapper">
      ...
      <div id="page-wrapper"> 
      		 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Cancel Reason Details</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example" style="font-size:13px;">
							        ...
							        <tbody>
							            <c:forEach items="${master}" var="cancelResone" varStatus="status">     
								  			<tr>
								            	<td><c:out value="${cancelResone.no}"/></td>
								                <td><c:out value="${cancelResone.name}"/></td>
								                <td><c:out value="${cancelResone.module}"/></td>
							                	<td><c:out value="${cancelResone.createdBy}"/></td>
								                <td><c:out value="${cancelResone.createdDate}"/></td>
							                	<td><c:out value="${cancelResone.modifyBy}"/></td>
							                	<td><c:out value="${cancelResone.modifyDate}"/></td>
								                <td><a href="#" onclick="editData('${cancelResone.id}','${cancelResone.name}','${cancelResone.module}')" data-toggle="modal" rel="tooltip" data-target="#exampleModal"  title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="deleteData('${cancelResone.id}')"  title="Inactive" data-toggle="modal" rel="tooltip" data-target="#deletbox"><span class="glyphicon glyphicon-trash"></span></a></td>
								            </tr>   
									    </c:forEach>
							        </tbody>
                                </table>
  ...
  <div class="modal-dialog" >
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">Edit cancel reason</h4>
      </div>
      <div class="modal-body">
        <form>
          ...
          <div class="form-group">
            <label for="message-text" class="control-label">cancel reason name</label>
             <input type="text" class="form-control" id="name" name="name">
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">cancel reason module</label>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">Add cancel reason</h4>
      </div>
      <div class="modal-body">
        <form>
        <div class="form-group">
        ...
        <div class="dataTable_wrapper">
           <table class="table table-striped table-bordered table-hover" id="dataTables-example1" style="font-size:13px;">
               <thead>
      		   ...
      		   <tbody>
          			<c:forEach items="${masters}" var="cancelResones" varStatus="status">     
 						<tr>
           					<td><c:out value="${cancelResones.no}"/></td>
               				<td><c:out value="${cancelResones.name}"/></td>
               				<td><c:out value="${cancelResones.module}"/></td>
		                	<td><c:out value="${cancelResones.createdBy}"/></td>
               				<td><c:out value="${cancelResones.createdDate}"/></td>
		                	<td><c:out value="${cancelResones.modifyBy}"/></td>
		                	<td><c:out value="${cancelResones.modifyDate}"/></td>
               				<td><a href="#" onclick="active('${cancelResones.id}')" title="Active" data-toggle="modal" rel="tooltip"><span style="color:green;font-size: 20px;" class="glyphicon glyphicon-ok-circle"></span></a></td>
          				</tr>   
    				</c:forEach>
      			</tbody>
           </table>
...
function editData(pid,data,module){
	$("#id").val(pid);
	$("#id1").val(pid);
	$("#name").val(data);
	$("#mod").val(module);
...
if($("#printName").val()=="" || $("#printName").val()=="null"){
	...
	$("#dataes1").html("Please enter cancel reason name");
	$('#alerts').modal('show');
	return false;
 }
if($("#mods").val()=="" || $("#mods").val()=="null"){
	$("#exampleModalLabelTitle2").html("Info");
	$("#dataes1").html("Please enter cancel reason module");
	$('#alerts').modal('show');
	return false;
 }else{
...
if($("#mod").val()=="" || $("#mod").val()=="null"){
	...
	$.ajax({
	    type: "Get",
	    url: "updateCancel",
	    data: "ID="+id+"&Name="+names+"&Mod="+mode,
	    success: function(data){
	    	if(data.success==true){
	    		$('#loading').modal('hide');
	    		$("#exampleModalLabelTitle1").html("Successfully");
...
function deleteData(id){
	$("#ids").val(id);
	$("#exampleModalLabelTitle").html("Inactive cancel Reason ");
